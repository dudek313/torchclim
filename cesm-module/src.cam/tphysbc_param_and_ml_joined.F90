...
subroutine tphysbc_joined (ztodt,   pblht,   tpert,   qpert,            &
...
!-----------------------------------------------------------------------
   ...
   use physics_types,   only: physics_state, physics_tend, physics_ptend, physics_update, physics_ptend_init, &
                              physics_state_copy, physics_tend_init
   ...
   use export_state,    only: record_state_before, record_state_after, physics_tend_copy, record_state_after_additional
...
!  pass macro to micro
   ...
   !GAIA: Used to run the ML/AI model side by side to the parametriztion without
   !affecting the actual run.
   real(r8) :: ml_fsns(pcols)                   ! Surface solar absorbed flux
   real(r8) :: ml_fsnt(pcols)                   ! Net column abs solar flux at model top
   real(r8) :: ml_flns(pcols)                   ! Srf longwave cooling (up-down) flux
   real(r8) :: ml_flnt(pcols)                   ! Net outgoing lw flux at model top
   real(r8) :: ml_fsds(pcols)                   ! Surface solar down flux
   real(r8) :: ml_flds(pcols)                   ! Surface solar down flux
   real(r8) :: ml_prect(pcols)                   
   real(r8) :: ml_precc(pcols)                  
   real(r8) :: ml_precl(pcols)                  
   real(r8) :: ml_precsc(pcols)                 
   real(r8) :: ml_precsl(pcols)                 
   real(r8) :: ml_soll(pcols)                 
   real(r8) :: ml_sols(pcols)                 
   real(r8) :: ml_solld(pcols)                 
   real(r8) :: ml_solsd(pcols)                 
   real(r8) :: ml_srfrad(pcols)                 
   real(r8) :: ml_solin(pcols)                 
   type(physics_state) :: ml_state
   type(physics_tend ) :: ml_tend
   !type(pbuf_fld),      intent(inout) :: pbuf(pbuf_size_max)
   type(cam_out_t)     :: ml_cam_out
   type(physics_ptend) :: ml_ptend            ! indivdual parameterization tendencies
...
!===================================================
   ...
   ml_fsns(:ncol) = fsns(:ncol)
   ml_fsnt(:ncol) = fsnt(:ncol)
   ml_flns(:ncol) = flns(:ncol)
   ml_flnt(:ncol) = flnt(:ncol)
   ml_fsds(:ncol) = fsds(:ncol)
   ml_fsns(:ncol) = fsns(:ncol)
   ml_fsns(:ncol) = fsns(:ncol)
   ml_fsns(:ncol) = fsns(:ncol)

   call physics_state_copy( state, ml_state )   ! Copy state to local ml_state.
   call physics_ptend_init( ml_ptend )
   call physics_tend_init( ml_tend )
   call physics_tend_copy( initial_tend, ml_tend )

   call ml_model_tend ( ztodt, ml_state, ml_ptend,  pbuf, &
        ml_fsns,    ml_fsnt,    ml_flns,    ml_flnt, ml_fsds, ml_flds, ml_solin, &
        cam_in, ml_cam_out, &
        ml_prect, ml_precc, ml_precl, &
        ml_precsc, ml_precsl, &
        ml_srfrad, ml_soll, ml_solld, ml_sols, ml_solsd, &
        relhum )

   call physics_update (ml_state, ml_tend, ml_ptend, ztodt)
   !call check_energy_chng(state, tend, "mlmodel", nstep, ztodt, zero, zero, zero, net_flx)

   call record_state_after(ml_state, ml_tend, ztodt, initial_state, initial_tend)
   call record_state_after_additional(state, ml_prect, ml_precc, ml_precl, ml_precsc, ml_precsl)
